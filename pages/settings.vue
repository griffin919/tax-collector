...
<div v-if="activeTab === 'customers'" class="bg-white rounded-lg shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="md:col-span-2">
          <input
            ...
            type="text"
            placeholder="Ghana Card Number (GHA-123456789-0)"
            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            required
          />
        </div>
  ...
  <div v-if="customers.length" class="divide-y">
    ...
    >
      <div>
        <h3 class="font-medium">{{ customer.firstName }} {{ customer.lastName }}</h3>
        <p class="text-sm text-gray-600">Ghana Card: {{ customer.ghanaCard }}</p>
        <p class="text-sm text-gray-600">
          Phone: {{ customer.phoneNumber }} | DOB: {{ formatDate(customer.dateOfBirth) }}
        </p>
      </div>
      <div class="flex gap-2">
        <button
    ...
    <div class="p-6">
      ...
      <form @submit.prevent="handleUpdateCustomer" class="space-y-4">
        <div>
          ...
          <input
            v-model="editingCustomer.firstName"
            type="text"
            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            required
          />
        ...
        <div>
          ...
          <input
            v-model="editingCustomer.lastName"
            type="text"
            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            required
          />
        ...
        <div>
          ...
          <input
            v-model="editingCustomer.dateOfBirth"
            type="date"
            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            required
          />
        ...
        <div>
          ...
          <input
            v-model="editingCustomer.ghanaCard"
            type="text"
            class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            required
          />
        ...
        <div class="flex justify-end gap-2">
          <button
            type="button"
            @click="isEditCustomerModalOpen = false"
...
// Initialize data
onMounted(async () => {
  await Promise.all([loadTaxTypes(), loadUsers(), loadCustomers()])
})
...
const handleUpdateCustomer = async () => {
  try {
    // Validate Ghana Card format
    if (!validateGhanaCard(editingCustomer.value.ghanaCard)) {
      const toast = useToast()
      toast.add({
        title: 'Error',
        description: 'Invalid Ghana Card format. Use GHA-123456789-0 format.',
    ...
    const result = await updateCustomer(editingCustomer.value.id, {
      ...
      dateOfBirth: editingCustomer.value.dateOfBirth,
      phoneNumber: editingCustomer.value.phoneNumber,
      ghanaCard: editingCustomer.value.ghanaCard
    })
